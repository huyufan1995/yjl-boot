$(function () {
    $("#jqGrid").jqGrid({
        ...
        colModel: [			
			...
			{ label: '分享时间', name: 'shareTime', index: 'share_time', width: 80 }/*,
			{
...
var vm = new Vue({
	...
	data:{
		...
		ruleValidate: {
		                ...
		                { required: true, message: '请输入分享人会员ID' }
		            ], 																shareTime: [
	...
	methods: {
		...
		saveOrUpdate: function (event) {
			this.$refs['caseShare'].validate((valid) => {
                if (valid) {
                	var url = vm.caseShare.id == null ? "../caseshare/save" : "../caseshare/update";
        			$.ajax({
        				type: "POST",
        			    url: url,
        			    contentType: "application/json",
        			    data: JSON.stringify(vm.caseShare),
        			    success: function(r){
        			    	if(r.code === 0){
        			    		vm.reload();
        			    		vm.showModal = false;
		...
		getInfo: function(id){
			//$.get("../caseshare/info/" + id, function(r){
            //    vm.caseShare = r.caseShare;
            //});
            
            $.ajax({
				type : "GET",
				async: false,
				url : "../caseshare/info/" + id,
				success : function(r) {
					vm.caseShare = r.caseShare;
				}
			});
		...
		reload: function (event) {
			...
			$("#jqGrid").jqGrid('setGridParam',{ 
				postData:{"shareName": vm.q.shareName, "sdate":vm.q.sdate, "edate":vm.q.edate},
                page:page
            }).trigger("reloadGrid");
		},
		formateDate (val){
