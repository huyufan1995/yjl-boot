$(function () {
    $("#jqGrid").jqGrid({
        url: '../clienteletag/list',
        datatype: "json",
        colModel: [			
			{ label: 'id', name: 'id', index: 'id', width: 50, key: true },
			{ label: '标签ID', name: 'tagId', index: 'tag_id', width: 80 },
			{ label: '标签内容', name: 'tagName', width: 80 },
...
var vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {
			this.$refs['clienteleTag'].validate((valid) => {
                if (valid) {
                	var url = vm.clienteleTag.id == null ? "../clienteletag/save" : "../clienteletag/update";
        			$.ajax({
        				type: "POST",
        			    url: url,
        			    contentType: "application/json",
        			    data: JSON.stringify(vm.clienteleTag),
        			    success: function(r){
        			    	if(r.code === 0){
        			    		vm.reload();
        			    		vm.showModal = false;
		...
		getInfo: function(id){
			//$.get("../clienteletag/info/" + id, function(r){
            //    vm.clienteleTag = r.clienteleTag;
            //});
            
            $.ajax({
				type : "GET",
				async: false,
				url : "../clienteletag/info/" + id,
				success : function(r) {
					vm.clienteleTag = r.clienteleTag;
				}
			});
