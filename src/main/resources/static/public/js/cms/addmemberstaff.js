$(function () {
    $("#jqGrid").jqGrid({
        ...
        datatype: "json",
        colModel: [
			{ label: '会员开通时间', name: 'ctime', index: 'ctime', width: 80 },
			...
			},
			{
                ...
                formatter: function (value, options, row) {
                	var dom ="";
					dom +="<button type='button' class='ivu-btn ivu-btn-primary' onclick='edit("+row.id+")'><i class='ivu-icon ivu-icon-minus'></i><span>增加子账号数量</span></button>&nbsp;";
		...
		saveOrUpdate: function (event) {
			this.$refs['member'].validate((valid) => {
                if (valid) {
                	...
                	var url = vm.member.id == null ? "../member/saveVipMember" : "../member/updateVipMebmerStaff";
        			$.ajax({
        				type: "POST",
        			    url: url,
