$(function () {
    $("#jqGrid").jqGrid({
        ...
        colModel: [			
			...
			{ label: '图标', name: 'image', index: 'image', width: 80 },
			{ label: '描述', name: 'describe', index: 'describe', width: 80 },
			{ label: '提示', name: 'hint', index: 'hint', width: 80 },
			{
	...
	methods: {
		...
		handleSuccess1 (res, file) {
			...
			if(res.code != 500){
				...
				vm.iconImgSrc = res.data.url;
				vm.functions.image = vm.iconImgSrc;
			}else{
				this.$Message.success('大小超过3M!');
			}
		},
		saveOrUpdate: function (event) {
			this.$refs['functions'].validate((valid) => {
                if (valid) {
                	var url = vm.functions.id == null ? "../functions/save" : "../functions/update";
        			$.ajax({
        				type: "POST",
        			    url: url,
        			    contentType: "application/json",
        			    data: JSON.stringify(vm.functions),
        			    success: function(r){
        			    	if(r.code === 0){
        			    		vm.reload();
        			    		vm.showModal = false;
		...
		getInfo: function(id){
			//$.get("../functions/info/" + id, function(r){
            //    vm.functions = r.functions;
            //});
            
            $.ajax({
				type : "GET",
				async: false,
				url : "../functions/info/" + id,
				success : function(r) {
					vm.functions = r.functions;
				}
			});
