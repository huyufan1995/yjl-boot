            ...
            colModel: [
                ...
                { label: '图片集', name: 'images', index: 'images', width: 80,
                    formatter: function (value, options, row) {
                        if(value != null){
                            return "<a class='btn btn-info' onclick='casusData(\""+row.id+"\")'>"+报名数据+"</a>";
                        }else{
                            return "<span></span>";
                        }
                    }
...
var vm = new Vue({
	...
	created: function(){
		...
		$.ajax({
			...
			success: function(r){
				var obj = r.case.details;
				var casusData = JSON.parse(obj);
				var list = casusData.list;
