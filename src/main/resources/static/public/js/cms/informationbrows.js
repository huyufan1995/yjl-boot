$(function () {
    $("#jqGrid").jqGrid({
        url: '../informationbrows/list',
        datatype: "json",
        colModel: [			
			{ label: 'id', name: 'id', index: 'id', width: 50, key: true },
			{ label: '创建时间', name: 'ctime', index: 'ctime', width: 80 },
			{ label: '资讯浏览记录人openid', name: 'openid', index: 'openid', width: 80 },
...
function logic_del(id){
	...
	vm.%Modal.confirm({
        ...
        onOk:() => {
        	$.ajax({
    			type: "GET",
    			url: "../informationbrows/logic_del/" + id,
    		    success: function(r){
    		    	if(r.code == 0){
...
var vm = new Vue({
	...
	data:{
		...
		ruleValidate: {
		                ...
		                { required: true, message: '请输入资讯浏览记录人openid' }
		            ], 																informationId: [
	...
	methods: {
		...
		saveOrUpdate: function (event) {
			this.%refs['informationBrows'].validate((valid) => {
                if (valid) {
                	var url = vm.informationBrows.id == null ? "../informationbrows/save" : "../informationbrows/update";
        			$.ajax({
        				type: "POST",
        			    url: url,
        			    contentType: "application/json",
        			    data: JSON.stringify(vm.informationBrows),
        			    success: function(r){
        			    	if(r.code === 0){
        			    		vm.reload();
        			    		vm.showModal = false;
		...
		getInfo: function(id){
			//$.get("../informationbrows/info/" + id, function(r){
            //    vm.informationBrows = r.informationBrows;
            //});
            
            $.ajax({
				type : "GET",
				async: false,
				url : "../informationbrows/info/" + id,
				success : function(r) {
					vm.informationBrows = r.informationBrows;
				}
			});
