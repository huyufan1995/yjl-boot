$(function () {
    $("#jqGrid").jqGrid({
        url: '../templateapplyuserecord/list',
        datatype: "json",
        colModel: [			
...
function logic_del(id){
	...
	vm.$Modal.confirm({
        ...
        onOk:() => {
        	$.ajax({
    			type: "GET",
    			url: "../templateapplyuserecord/logic_del/" + id,
    		    success: function(r){
    		    	if(r.code == 0){
...
var vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {
			this.$refs['templateApplyUseRecord'].validate((valid) => {
                if (valid) {
                	var url = vm.templateApplyUseRecord.id == null ? "../templateapplyuserecord/save" : "../templateapplyuserecord/update";
        			$.ajax({
        				type: "POST",
        			    url: url,
        			    contentType: "application/json",
        			    data: JSON.stringify(vm.templateApplyUseRecord),
        			    success: function(r){
        			    	if(r.code === 0){
        			    		vm.reload();
        			    		vm.showModal = false;
		...
		del: function (event) {
	        ...
	        onOk:() => {
	        	$.ajax({
					type: "POST",
				    url: "../templateapplyuserecord/delete",
		...
		getInfo: function(id){
			//$.get("../templateapplyuserecord/info/" + id, function(r){
            //    vm.templateApplyUseRecord = r.templateApplyUseRecord;
            //});
            
            $.ajax({
				type : "GET",
				async: false,
				url : "../templateapplyuserecord/info/" + id,
				success : function(r) {
					vm.templateApplyUseRecord = r.templateApplyUseRecord;
				}
			});
